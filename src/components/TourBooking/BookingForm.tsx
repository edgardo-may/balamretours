...
import { Calendar, Users, MapPin, User, Mail, Phone, ShoppingCart, Loader2, CheckCircle2 } from "lucide-react";
import type { TourWithDetails, Reservation } from "../../types/tour";
  ...
  if (success) {
    return (
      <div className="bg-white rounded-3xl p-8 shadow-xl border border-teal-100 text-center animate-in fade-in zoom-in duration-300">
        ...
        <h3 className="text-2xl font-black text-slate-900 mb-2">¡Reserva Exitosa!</h3>
        <p className="text-slate-500 mb-6">Tu aventura está lista. Hemos enviado un correo con los detalles.</p>
        <div className="bg-slate-50 p-4 rounded-2xl mb-8">
          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block mb-1">Tu Folio de Confirmación</span>
  ...
  return (
    <div className="lg:sticky lg:top-8 flex flex-col gap-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormData({ ...formData, ninos: Number(e.target.value) })}
                className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-medium focus:ring-2 focus:ring-teal-500 outline-none appearance-none"
              >
                {[0, 1, 2, 3, 4, 5].map((n) => (
                  <option key={n} value={n}>{n}</option>
          ...
          <div className="bg-slate-900 rounded-2xl p-6 text-white space-y-3">
             <div className="flex justify-between text-slate-400 text-xs font-bold uppercase">
               <span>Adultos ({formData.adultos} x ${adultPrice})</span>
               <span>${summary.totalAdults}</span>
             </div>
             {formData.ninos > 0 && (
               <div className="flex justify-between text-slate-400 text-xs font-bold uppercase">
                 <span>Niños ({formData.ninos} x ${childPrice})</span>
                 <span>${summary.totalChildren}</span>
               </div>
             )}
             <div className="pt-3 border-t border-slate-800 flex justify-between items-center">
                <span className="text-sm font-bold">Total a pagar</span>
                <span className="text-2xl font-black text-teal-400">${summary.total}</span>
             </div>
          </div>
          ...
          >
            {loading ? (
              ...
              <span className="text-sm font-black uppercase tracking-widest">Confirmar Reserva</span>
            )}
          </Button>
        </form>
      </div>
      ...
      <p className="text-center text-[10px] text-slate-400 font-medium px-4">
        Al confirmar, aceptas nuestros términos y condiciones. Los precios están en pesos mexicanos (MXN).
      </p>
    </div>
  );
};
