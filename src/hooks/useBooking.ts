...
interface CreateReservationInput extends Omit<Reservation, 'id' | 'folio_reservacion' | 'estado' | 'created_at'> {
...
export function useBooking() {
  ...
  const createReservation = async (reservation: CreateReservationInput) => {
    try {
      ...
      const { nombre_cliente, email_cliente, telefono_cliente, tour_nombre, ...reservationData } = reservation;
      ...
      if (email_cliente) {
        sendBookingConfirmation({
          ...
          if (!result.success) {
            console.warn('[useBooking] Email confirmation failed:', result.error);
          } else {
            console.info('[useBooking] Confirmation email sent:', result.email_id);
          }
        });
      }
